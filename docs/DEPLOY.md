...
## Тека вкладень на томі

Том Fly монтується в `/data` від імені root, а застосунок в образі працює
під `nextjs` (uid 1001) — сам він теку в такому томі створити не може.
Готує її `docker-entrypoint.sh`: поки контейнер ще root, створює
`$UPLOAD_DIR`, віддає її користувачу й лише тоді запускає застосунок
через `su-exec`.

Без цього збереження будь-якого файла — обкладинки допису, вкладення —
падало з `EACCES`, а на екрані з'являлась порожня сторінка
